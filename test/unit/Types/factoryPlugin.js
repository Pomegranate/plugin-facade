...
/**
 *
 * @module factoryPlugin
 */

tap.test('Factory Plugin', (t) => {
  let pluginData = getPlugin('common/plugins/', 'factory')
  ...
  t.test('Initialize', (t) => {
    SP.initialize({FrameworkDI, PluginDI})
      .then((p) => {
        t.equal(p.IProperty('type'), 'factory', "Interface has correct type.")
        t.done()
      })
