...
function getExternalOptions(pluginSettings){
  /**
   * TODO - Lookup namespace plugins from pluginSettings.namespaceConfigs
  ...
  if(pluginSettings.path){
    ...
    let nsFiles = pluginSettings.namespaceConfigs[this.namespace] ? pluginSettings.namespaceConfigs[this.namespace] : []
    let someFiles = this.namespace ? nsFiles : pluginSettings.files
    let settingMatch = _.some(someFiles,function(file){
      return file === configName
    })

    if(settingMatch){
      let settingsThing = this.namespace ? require(path.join(pluginSettings.path, this.namespace, configName)) :require(path.join(pluginSettings.path, configName))
