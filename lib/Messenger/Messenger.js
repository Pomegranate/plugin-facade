...
const buildErrorOutput = _fp.compose(_fp.join('\n'),_fp.map(i => i.join('\n')),_fp.filter(i => i.length))
/**
 *
 * @module Messenger
 */
...
class Messenger {
  ...
  outputErrors(errors){
    let o = buildErrorOutput(errors)
    this.PluginLogger.error(`Encountered the following errors: \n${o}`)
  }
  ...
  initializing() {
    ...
    this.PluginLogger.log(`Initializing version ${this.Version} ${this.Type} plugin.`, 1)
  }
